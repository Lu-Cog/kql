		...
		<view :class="{ appintAlertContent: true, aCShow: isShow }" @click.stop="_stopFunc">
			...
			<view class="alertTimebox">
				<view class="right_box">
					<view @click="_changeTime(index)" :class="{ active: item.checked }" v-for="(item, index) in activeTimeArr" :key="item.time" :style="item.time=='12:30'?'color:#eee':''">
...
export default {
	...
	methods: {
		...
		_handleData() {
			...
			console.log(selectTime,'123');
			_data.date = selectTime;
