	...
	export default {
		...
		methods: {
			...
			isWorkDay(y, m, d) {
				...
				if (showTime-curTime>=0 && showTime-curTime<86400000) {
					return true;
				} else{
					return false;
				}
			...
			selectOne(i, event) {
				...
				let sYear = new Date(curTime).getFullYear()
				let sYue = (new Date(curTime).getMonth() + 1)<10?'0'+(new Date(curTime).getMonth() + 1):(new Date(curTime).getMonth() + 1)
				let sDay = new Date(curTime).getDate()<10?'0'+new Date(curTime).getDate():new Date(curTime).getDate()
				let sDate = sYear + '-' + sYue + '-' + sDay
				console.log(sDate,date);
				console.log(selectD-curTime,Math.abs(selectD-curTime)>86400000);
				if (!(selectD-curTime<86400000 && selectD-curTime>0)) {
